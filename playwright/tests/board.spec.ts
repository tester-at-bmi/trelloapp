...
test.describe('Boards', () => {
    ...
    test('Adds a new list', async ({ page, browserName }) => {
        // Visit board page.
        ...
        await page.waitForSelector('[data-cy=add-list-options]', { state: 'hidden', timeout: 4000 });
        ...
        await page.waitForSelector('[data-cy=list]', { state: 'visible', timeout: 4000 });
        ...
        if (browserName === 'chromium') {
            ...
            expect(await page.evaluate(() => navigator.clipboard.readText())).toContain('id');
            expect(await page.evaluate(() => navigator.clipboard.readText())).toContain('created');
            expect(await page.evaluate(() => navigator.clipboard.readText())).toContain('boardId');
        }
        await page.locator('span').getByText('Delete list').first().click({ timeout: 4000 });
        await page.waitForSelector('[data-cy=list]', { state: 'hidden', timeout: 4000 });
    });
